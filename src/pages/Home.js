	...
	async componentDidUpdate(prevProps) {
		if (prevProps.location !== this.props.location) {
			if (this.props.location.pathname === "/search") {
				const searchResults = await api.busqueda(
					this.props.location.searchParams
				);
				if (searchResults) this.setState({ latestNews: searchResults });
			} else if (this.props.location.categoryId) {
				const categoryId = this.props.location.categoryId;
				const categoryNews = await api.category(categoryId);

				if (categoryNews) {
					this.setState({
						latestNews: categoryNews.slice(0, 10),
						isLoading: false
					});
				}
			} else {
